...
function AddExtraClips(){ //adds the parts that aren't cut (based from the cuts) and returns the added clips and cuts as new array
  ...
  for (i = 0; i < cuts.length; i++){
    if (i - 1 > -1){
      cutsToAdd.push({"start": cuts[i - 1].end, "end": cuts[i].start, "enabled": true, "offset": cuts[i - 1].offset})
    }
  }
  if (cuts[cuts.length - 1].end != currentDuration) cutsToAdd.push({"start": cuts[cuts.length - 1].end, "end": currentDuration, "enabled": true, "offset": cuts[cuts.length - 1].offset})
...
function AddExtraAssets(){ //adds the extra assets (formats) for a video (note each asset-clip uses one format which is currently the 720p one)
  ...
  let vals = [{"width": "1920", "name": "FFVideoFormat1080p30", "height": "1080"}, {"width": "1280", "name": "FFVideoFormat720p30", "height": "720"}]
